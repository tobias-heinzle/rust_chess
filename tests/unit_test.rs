...
mod tests {
    ...
    fn quiescence_checkmate() {
        let board =
            chess::Board::from_str("R6k/6rp/5B2/8/8/8/7P/7K b - - 0 3").expect("Invalid position");
        let alpha = 0;
        let beta = 100;
        ...
        assert_eq!(result, -(rust_chess::config::INFINITY));
    }

    #[test]
    fn quiescence_stalemate() {
        let board =
            chess::Board::from_str("7k/6Rp/7B/8/8/8/7P/7K b - - 0 1").expect("Invalid position");
        let alpha = 0;
        let beta = 100;
        let mut context = setup_test_context(board);
    ...
    fn uci_read_position() {
        let command: Vec<&str> = "fen 7k/6Rp/7B/8/8/8/7P/7K w - - 0 1 moves g7h7 h8h7 h6g7"
            .split(" ")
            .collect();
    ...
    fn test_extensions_in_check() {
        let board = chess::Board::from_str("r7/ppp3k1/6b1/3pQ3/5R2/1P4P1/P5PP/6K1 b - - 4 43")
    ...
    fn test_extensions_not_in_check() {
        let board = chess::Board::from_str("r5k1/ppp5/5Qb1/3p4/5R2/1P4P1/P5PP/6K1 b - - 6 44")
            .expect("Invalid position");
