...
mod tests {
    ...
    #[test]
    fn mate_in_three_0() {
        let board = chess::Board::from_str("r5rk/5p1p/5R2/4B3/8/8/7P/7K w - - 0 1")
        ...
        assert_eq!(result.0, rust_chess::config::INFINITY - 2);
    }
    ...
    #[test]
    fn repetition_draw() {
        let board = chess::Board::from_str("kr4QQ/6QQ/6QQ/6QQ/6QQ/5PPP/4q3/7K b - - 0 1")
            .expect("Invalid position");
        let max_depth = 6;
        let mut context = setup_test_context(board);

        let result = context.root_search(max_depth);
        assert_eq!(result.0, rust_chess::config::DRAW);
    }

    #[test]
    fn avoid_repetition() {
        let command: Vec<&str> =
            "fen R5k1/8/6pp/5p2/P4P2/r3P3/5KPP/8 b - - 1 43 moves g8g7 a8a7 g7g8"
                .split(" ")
                .collect();
    ...
    fn is_repetition() {
        let command: Vec<&str> =
            "fen R5k1/8/6pp/5p2/P4P2/r3P3/5KPP/8 b - - 1 43 moves g8g7 a8a7 g7g8 a7a8"
                .split(" ")
                .collect();
        let position = rust_chess::uci::change_position(&command[0..]);
        let fen = position.board.to_string();
        ...
        assert!(result.0 == rust_chess::config::DRAW);
    }

    #[test]
    fn find_critical_endgame_move_part_1() {
        let board = chess::Board::from_str("8/k7/3p4/p2P1p2/P2P1P2/8/8/K7 w - - 0 1")
            .expect("Invalid position");
        let max_depth = 18;
        let mut context = setup_test_context(board);
    ...
    fn find_critical_endgame_move_part_2() {
        let board = chess::Board::from_str("8/1k6/3p4/p2P1p2/P2P1P2/8/8/1K6 w - - 2 2")
            .expect("Invalid position");
        let max_depth = 18;
        let mut context = setup_test_context(board);
    ...
    fn find_critical_endgame_move_part_3() {
        let board = chess::Board::from_str("8/2k5/3p4/p2P1p2/P2P1P2/8/8/2K5 w - - 0 1")
            .expect("Invalid position");
        let max_depth = 18;
        let mut context = setup_test_context(board);
    ...
    #[test]
    fn mate_in_four() {
        let board = chess::Board::from_str("r4r1k/1R1R2p1/7p/8/8/3Q1Ppq/P7/6K1 w - - 0 1")
            .expect("Invalid position");
        let max_depth = 8;
        let mut context = setup_test_context(board);
        ...
        assert_eq!(result.0, rust_chess::config::INFINITY - 3);
    }

    #[test]
    fn mate_in_negative_one() {
        let board = chess::Board::from_str("6k1/5ppp/5n2/2p2P2/2P1p3/6q1/8/1B1r1K1R w - - 2 45")
            .expect("Invalid position");
        let max_depth = 4;
        let mut context = setup_test_context(board);
        ...
        assert_eq!(result.0, -rust_chess::config::INFINITY + 1);
    }

    #[test]
    fn mate_in_negative_two() {
        let board = chess::Board::from_str("6k1/5ppp/5n2/2p2P2/2Prp3/8/2B1K1q1/7R w - - 2 45")
            .expect("Invalid position");
        let max_depth = 4;
        let mut context = setup_test_context(board);
    ...
    fn blunder_vs_myopic_bot_1() {
        let board = chess::Board::from_str("4r1k1/p2nB2p/4pbp1/1Np5/8/1P3P2/P3KP1P/3R4 w - - 6 26")
            .expect("Invalid position");
        let max_depth = 8;
        let mut context = setup_test_context(board);
    ...
    fn blunder_vs_myopic_bot_2() {
        let board = chess::Board::from_str("8/p2N1nk1/8/4pKP1/1P5p/8/P7/8 b - - 1 47")
            .expect("Invalid position");
        let max_depth = 10;
        let mut context = setup_test_context(board);
    ...
    fn enemy_blunder_of_myopic_bot() {
        let board = chess::Board::from_str("8/p7/3n2k1/4K1P1/1P6/6N1/P6p/8 b - - 3 51")
            .expect("Invalid position");
        let max_depth = 11;
        let mut context = setup_test_context(board);
    ...
    //     assert_eq!(result.0, rust_chess::config::INFINITY  - 4);
    ...
    //     assert_eq!(result.0, rust_chess::config::INFINITY - 5);
    // }
